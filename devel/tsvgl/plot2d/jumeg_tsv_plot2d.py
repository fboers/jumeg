...
class JuMEG_TSV_PLOT2D_WX_GL_CANVAS(JuMEG_TSV_PLOT2D_WX_GL_CANVAS_BASE):
      ...
      def InitGL(self):
          ...
          self.is_initGL = self.gl_plot2d.is_initGL
      ...
      def OnDraw(self,size_mm=None):
          ...
          if not self.gl_plot2d.raw:
             self.gl_plot2d.splash()
          else :
      ...
      def update(self,raw=None,n_channels=None,n_cols=None): #,do_scroll_channels=True,do_scroll_time=True):
           ...
           self.SetCurrent(self.context)
           if raw : 
              #print self.gl_plot2d.__dict__()
              print "Update :" + self.gl_plot2d.test_str   
              self.gl_plot2d.init_raw_data(raw=raw,n_channels=n_channels,n_cols=n_cols)                
           ...
           if self.gl_plot2d.opt.do_scroll:
              ...
              #self.Refresh()
              #self.plot_axis.range_max = self.gl_plot2d.timepoints[-1]
              #self.plot_axis.range_min = self.gl_plot2d.timepoints[0]
           ...
           #self.SwapBuffers()   
           self.Refresh()
           return self.gl_plot2d.timepoints[0],self.gl_plot2d.timepoints[-1]
       
      def SetCursor(self,xpos=None,ypos=None):
          self.gl_plot2d.SetCursor(xpos=xpos,ypos=ypos)
          self.SwapBuffers()  
...
class JuMEG_TSV_PLOT2D_PANEL(wx.Panel):
      ...
      def __init__(self, parent,verbose=False):
         ...
         #---toDo new CLS 
          self.XAxisRuler = RC.RulerCtrl(self, orient=wx.HORIZONTAL,style=wx.SUNKEN_BORDER)
          self.XAxisRuler.SetRange(0.0,1.0)
          self.XAxisRuler.SetFlip(True)
          self.XAxisRuler.SetLabelEdges(False)  
          rcid0=wx.NewId()
          rcid1=wx.NewId()
          
          self.XAxisIndi0 = self.XAxisRuler.AddIndicator(rcid0, 0.0)
          self.XAxisIndi1 = self.XAxisRuler.AddIndicator(rcid1, 0.0)
          
          #self.Bind(RC.EVT_INDICATOR_CHANGING,self.XAxisRulerOnIndicatorChanging, id=self.XAxisIndi0.GetId(),id2=self.XAxisIndi1.GetId() )
          self.Bind(RC.EVT_INDICATOR_CHANGED, self.XAxisRulerOnIndicatorChanged,id=rcid0,id2=rcid1 )
      ...
      def XAxisRulerOnIndicatorChanging(self, event):

        # Show how to block a changing indicator
          self.Write("NoNoNoNoNo! You Can't Change My Value!")
          return

      def XAxisRulerOnIndicatorChanged(self, evt):
          
          self.Plot2DCanvas.SetCursor(xpos=evt.GetValue())
          
          print("New Indicator Value Is: %0.2f"%evt.GetValue())
          evt.Skip()
      ...
      def update(self,raw=None,n_channels=1,n_cols=1):
          # self.Plot2DCanvas.plot2d.Hruler = self.XAxisRuler
          print"update plot"
          
          tmin,tmax= self.Plot2DCanvas.update(raw=raw,n_channels=n_channels,n_cols=n_cols)
          print "DONE update Plot2 min/max:"
          self.XAxisRuler.SetRange(tmin,tmax) 
            
              
      def __ApplyLayout(self):
          vbox = wx.BoxSizer(wx.VERTICAL)    
          #vbox.Add(self._plot2d_panel, 1, wx.ALIGN_CENTER|wx.ALL|wx.EXPAND,10)
          vbox.Add(self.Plot2DCanvas, 1, wx.ALIGN_CENTER|wx.ALL|wx.EXPAND,1)
          vbox.Add(self.XAxisRuler, 0, wx.ALIGN_CENTER|wx.ALL|wx.EXPAND,1)
