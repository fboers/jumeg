   ...
   #---> FIF / MNE path        : /mnt/meg_store1/exp/INTEXT/mne/211063/INTEXT01/181115_1120/1/
    ...
    if not opt.overwrite:
       if os.path.isfile(fif_out):
          if opt.verbose:
             jb.Log.info(["===> 4D/BTI file   : "+fpdf,
                         " --> FIF / MNE file: {} size [Mb]: {}".format(os.path.basename(fif_out), os.path.getsize(fif_out)/1024.0**2 ),
                         " --> FIF / MNE path: {}".format(os.path.dirname(fif_out)),
                         "  -> !!! FIF File exist will not overwrite [overwrite={}]".format(opt.overwrite),
                         "===> Done"
                         ])
          return
...
def get_args(self):
  ...
  #--- flags
    parser.add_argument("-prel",  "--preload",        action="store_true",help=h_preload)
    parser.add_argument("-sort",  "--sort_by_ch_name",action="store_true",help=h_sort_by_ch_name)
    parser.add_argument("-rename","--rename_channels",action="store_true",help=h_rename_channels)
    parser.add_argument("-conv",  "--convert",        action="store_true",help=h_convert)
    parser.add_argument("-overwrite","--overwrite",   action="store_true",help=h_overwrite)
    parser.add_argument("-save",  "--save",           action="store_true",help="save as fif file")
  ...
  #--- init flags
    parser.set_defaults(preload=True,sort_by_ch_name=True,rename_channels=True,convert=True,save=True,run=True)
...
def main(argv):
    ...
    if opt.verbose :
       msg = ["---> jumeg preproc import to fif: {}".format(__version__),
