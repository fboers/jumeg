...
class JuMEG_MergeMEEG_HiLoRate(object):
    ...
    def get_onset_and_events(self):
        ...
        """
        jb.Log.info(" --> call mne find events")
       ...
       #--- check  if no startcode  -> startcode not in/any  np.unique(ev[:, 2])
        if ( self.startcode in np.unique(ev[:, 2]) ):
           ...
           jb.Log.info(["---> ERROR no startcode found %d ".format(self.startcode),
                        "---> Events: ".format(np.unique(ev[:, 2]))])
           # assert ev_start_code_onset_idx,"ERROR no startcode found in events !!!"
          
        if self.verbose:
           jb.Log.info(["-" * 50,
               " --> Onset & Events Info : " + self.system,
               "  -> Onset index         : {}".format( ev_start_code_onset_idx)])
           if self.debug:
              jb.Log.debug(["  -> Onsets              :".format(ev_onsets),"-"*50])
...
class JuMEG_MergeMEEG(JuMEG_Base_IO):
    ...
    def check_data_drift(self):
        ...
        if self.verbose:
           ...
           if self.debug:
              ...
              "\n  -> Info MEG/EEG onsets:  code  t[s]",line])
              i=0
              dmeeg    =  np.zeros(meg_counts) 
              ...
              while i < meeg_counts:
                  ...
                  jb.Log.debug("->% 5d MEG code:% 7.1f t:% 10.4f  EEG code:% 7.1f t:% 10.4f div: % 7.5f" % (i,megcode,megonset,eegcode,eegonset,dmeeg[i]))
                  i+=1
           ...
           jb.Log.info([line,
              ...
              "     meg sampling periode [ms]: %0.6f" % (meg_samp_periode),line])
           if self.debug:
              jb.Log.debug(["     Diverence  abs(MEEG)",
                  "      AVG [ms]: % 3.5f"%(dmeeg_abs.mean()),
                  "      STD [ms]: % 3.5f"%(dmeeg_abs.std()),
                  "      MIN [ms]: % 3.5f"%(dmeeg_abs.min()),
                  "      MAX [ms]: % 3.5f"%(dmeeg_abs.max()),line])

        if ( dif_dt_last > meg_samp_periode ):
            jb.Log.error(" -->ERROR Data Drift ->last common meg/eeg event-code-onset is not in time resolution uncertainty\n -->MEG: %s\n -->EEG: %s" %( self.meg.filename,self.eeg.filename))
    ...
    def run(self):
        ...
        line="-"*50
        jb.Log.info(["---> Start JuMEG MEEG Merger",line])
       ...
       #--- load BV eeg
        jb.Log.info(" --> load EEG BrainVision data: " + self.eeg.filename)
        self.eeg.reset()
       ...
       # --- load meg
        jb.Log.info([line," --> load MEG data:             " + self.meg.filename])
        self.meg.reset()
      ...
      # --- filter meg / eeg data; no trigger
       ...
       # fif  Old style for our 4D ['EOG 001','ECG 001','EOG 002'] or New ['EEG 001','EEG 001','EEG 002']
        ...
        jb.Log.info([
               ...
               "  -> MEG Labels       : {}".format( meg_picks_eeg)])
        meg_picks_eeg.sort()  # inplace
        jb.Log.info([
               "  -> MEG Labels sorted: {}".format( meg_picks_eeg),
           ...
           #print("EEG ch: {} meg list: {}".format(eeg_ch,meg_picks_eeg))
            ...
            if eeg_ch in meg_picks_eeg:
               ...
               if not meg_chname:
                  ...
                  #continue
               jb.Log.info("  -> copy EEG channel: {} idx:{} to MEG channel: {}".format(eeg_ch, eeg_idx, meg_chname))

              #--- copy eeg downsapled data into raw
               jb.Log.info("meg_chname      : {}".format(meg_chname))
               jb.Log.info("meg_chname index: {} ".format(self.meg.raw.ch_names.index(meg_chname)))
               jb.Log.info("eeg_index       : {}".format(eeg_idx))
      ...
      # --- adjust data size to merged data block -> cut meg-data to meg-onset <-> meg-end tsls
        if self.do_adjust_data_size:
           ...
           jb.Log.info(["Eventcode adjusted: " + self.meg.stim_channel,
      ...
      #--- upate bads 
       ...
       #--- save meeg
        ...
        jb.Log.info("---> start saving MEEG data: "+ self.meeg_fname)
        if self.do_save:
           self.apply_save_mne_data(self.meg.raw,fname=self.meeg_fname,overwrite=True)
        else:
           jb.Log.info([" -->set Bads; bad-channels in meeg-raw file:",
                        self.picks.bads(self.meg.raw),line])
        
        jb.Log.info(["---> DONE merge BrainVision EEG data to MEG",line])
...
def apply_jumeg_merge_meeg(opt):
  ...
  #--- check for meg and eeg filename  
   ...
   if not fn_raw_list:
      jb.Log.error("!!! ERROR jumeg_merger_meeg: no meg and/or eeg file to process !!!")
      return         
...
def main(argv):
    ...
    if opt.verbose :
       msg=["---> jumeg preproc merger meeg: {}".format(__version__),
...
#--- 0815
#0815/M100/170629_0852/2/0815_M100_170629_0852_2_c,rfDC-raw.fif --feeg=0815_M100_002.vhdr
#0815/M100/170629_0852/3/0815_M100_170629_0852_3_c,rfDC-raw.fif --feeg=0815_M100_003.vhdr
#0815/M100/170629_0852/4/0815_M100_170629_0852_4_c,rfDC-raw.fif --feeg=0815_M100_004.vhdr
#0815/M100/170629_0852/5/0815_M100_170629_0852_5_c,rfDC-raw.fif --feeg=0815_M100_005.vhdr
#0815/M100/170629_0852/6/0815_M100_170629_0852_6_c,rfDC-raw.fif --feeg=0815_M100_006.vhdr
#0815/M100/170629_0852/7/0815_M100_170629_0852_7_c,rfDC-raw.fif --feeg=0815_M100_007.vhdr
#--- resting state
0815/M100/170629_1027/1/0815_M100_170629_1027_1_c,rfDC-raw.fif --feeg=0815_M100_restclosed.vhdr --startcode=128 --bads=MEG 007,MEG 142
#0815/M100/170629_1027/2/0815_M100_170629_1027_2_c,rfDC-raw.fif --feeg=0815_M100_restopen.vhdr --startcode=128
#
#--- 0816
#0816/M100/170706_0915/3/0816_M100_170706_0915_3_c,rfDC-raw.fif --feeg=0816_M100_001.vhdr 
#0816/M100/170706_0915/4/0816_M100_170706_0915_4_c,rfDC-raw.fif --feeg=0816_M100_002.vhdr 
#0816/M100/170706_0915/5/0816_M100_170706_0915_5_c,rfDC-raw.fif --feeg=0816_M100_003.vhdr 
#0816/M100/170706_0915/6/0816_M100_170706_0915_6_c,rfDC-raw.fif --feeg=0816_M100_004.vhdr 
#0816/M100/170706_0915/7/0816_M100_170706_0915_7_c,rfDC-raw.fif --feeg=0816_M100_005.vhdr 
#0816/M100/170706_0915/7/0816_M100_170706_0915_8_c,rfDC-raw.fif --feeg=0816_M100_006.vhdr 
#--- resting state
#0816/M100/170706_1030/1/0816_M100_170706_1030_1_c,rfDC-raw.fif --feeg=0816_M100_restclosed.vhdr
#0816/M100/170706_1030/2/0816_M100_170706_1030_2_c,rfDC-raw.fif --feeg=0816_M100_restopen.vhdr
#

#---  example cmd calls
#jumeg_merge_meeg.py -seeg /data/meg1/exp/M100/eeg/M100/ -smeg /data/meg1/exp/M100/mne/ -plist /data_meg1/exp/M100/doc/ -flist M100_merge_meeg.txt -sc 5 -b -v -r
'''
